...
function matchHelper(s: string, p: string, sindex: number, pindex: number, branch: number): boolean {
  ...
  if(pindex >= p.length && sindex < s.length || (sindex >= s.length && pindex < p.length-1 && !Array.from(p.slice(pindex)).every(x => x == "*"))) return (false);
  ...
  if(p[pindex] == "?") {
    return sindex < s.length && matchHelper(s, p, sindex+1, pindex+1,4);
  }
...
function isMatch(s: string, p: string): boolean {
  return matchHelper(s.replace(/\*+/g,"*")
, p, 0, 0, 0);
  //return false;
};